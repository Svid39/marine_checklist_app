...
### Улучшено ✨
- **Индикаторы загрузки:** Добавлены блокирующие модальные окна при длительных операциях (генерация PDF, обработка фото), чтобы пользователь видел процесс.
- **Очистка данных:** Реализовано автоматическое удаление файлов фотографий с устройства при удалении проверки или несоответствия.
- **Код:** Проведен полный рефакторинг, удалены отладочные принты, добавлена документация ко всем методам и классам.

### Исправлено 🛠️
- Исправлены предупреждения `deprecated_member_use` (замена `withOpacity`).
- Устранены потенциальные утечки памяти и ошибки асинхронности во всех экранах.
