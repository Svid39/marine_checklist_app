...
///
/// Инкапсулирует всю логику работы с пакетом `pdf`, предоставляя методы
/// для генерации отчетов по конкретной проверке и сводных отчетов по несоответствиям.
class PdfGeneratorService {
  /// Генерирует PDF-отчет для одного конкретного экземпляра чек-листа.
  ///
  /// Собирает всю информацию о проверке (метаданные, ответы, комментарии)
  /// и форматирует ее в виде документа с таблицами.
  Future<Uint8List> generateChecklistInstancePdf(
    ChecklistInstance instance,
    ChecklistTemplate template,
    UserProfile? userProfile,
  ) async {
    // Загрузка шрифта, поддерживающего кириллицу (для имен и названий), из ассетов.
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        // Хедер страницы (кроме первой) с названием чек-листа
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (context) => _buildPageHeader(
            context, 'Non-Conformity Report - Vessel: $targetShipName'),
        footer: (context) => _buildPageFooter(context),
        build: (pageContext) => [
          _buildDeficiencyReportHeader(targetShipName, userProfile),
          pw.SizedBox(height: 20),
          _buildDeficienciesTable(
              deficienciesForShip, instancesMap, templatesMap),
        ],
      ),
    );
    return pdf.save();
  ...
  /// Строит верхний колонтитул для страниц (отображается начиная со второй страницы).
  ...
  /// Строит нижний колонтитул с номерами страниц (Стр. X из Y).
  ...
  /// Строит "шапку" (заголовок и метаданные) для отчета по проверке.
          ...
          children: [
            pw.Column(
              ...
              children: [
                ...
                pw.Text(
                    'Inspector: ${instance.inspectorName ?? userProfile?.name ?? 'N/A'}'),
              ],
            ),
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                ...
                pw.Text(
                    'Completion date: ${formatDate(instance.completionDate)}'),
              ],
            ),
          ],
        ),
  ...
  pw.Widget _buildDeficiencyReportHeader(
    ...
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text('Non-Conformity Report',
            style: pw.TextStyle(fontSize: 20, fontWeight: pw.FontWeight.bold)),
        pw.Text('Vessel: $targetShipName',
            style: pw.TextStyle(fontSize: 16)),
        pw.SizedBox(height: 5),
        pw.Text('Inspector: ${userProfile?.name ?? 'N/A'}',
  ...
  pw.Widget _buildDeficienciesTable(
    ...
    final data = deficiencies.map((def) {
      ...
      if (instance != null && template != null) {
        sourceInfo =
            '${template.name} (item ${def.checklistItemId ?? 'N/A'}) from ${DateFormat('dd.MM.yy').format(instance.date)}';
      } else {
      ...
      return [
        ...
        def.assignedTo ?? 'N/A',
        def.dueDate != null
            ? DateFormat('dd.MM.yyyy').format(def.dueDate!)
            : 'N/A',
        sourceInfo,
  ...
  /// Вспомогательная функция для преобразования статуса несоответствия в строку (En).
  ...
  /// Вспомогательная функция для преобразования статуса ответа на пункт в строку (En).
