...
class _DeficiencyDetailScreenState extends State<DeficiencyDetailScreen> {
  ...
  Future<void> _selectDate({required bool isDueDate}) async {
    final initialDate = (isDueDate ? _dueDate : _resolutionDate) ?? DateTime.now();
  ...
  Future<void> _handleDeficiencyPhoto() async {
    ...
    try {
      ...
      if (instanceId != null) {
        final instance = Hive.box<ChecklistInstance>(instancesBoxName).get(instanceId);
        if (instance != null) {
      ...
      final newPath = await _compressAndSaveDeficiencyImage(pickedFile, contextShipName, contextPort, contextDate, contextIdentifier);

      if (!mounted) return;
      ScaffoldMessenger.of(context).removeCurrentSnackBar();

      if (newPath != null) {
        if (_photoPath != null && _photoPath!.isNotEmpty && _photoPath != newPath) {
          try {
  ...
  Future<String?> _compressAndSaveDeficiencyImage(XFile originalFile, String? shipName, String? port, DateTime? date, String identifier) async {
    try {
      ...
      final idPart = _sanitizeForFilename(identifier, defaultVal: 'ID_U', maxLength: 25);
      final datePart = date != null ? "${date.year}${date.month.toString().padLeft(2, '0')}${date.day.toString().padLeft(2, '0')}" : "DateU";
      final uniqueName = 'DEF_${shipPart}_${portPart}_${idPart}_${datePart}_${DateTime.now().millisecondsSinceEpoch}.$ext';
