...
class _ChecklistExecutionScreenState extends State<ChecklistExecutionScreen> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _template = template;
          if (_template != null) {
            _groupChecklistItems(_template!.items);
          }
          _isLoading = false;
        });
      }
  ...
  Future<void> _showCommentDialog(ChecklistItem item, ChecklistItemResponse currentResponse) async {
    final TextEditingController commentController = TextEditingController(text: currentResponse.comment ?? '');
  ...
  Future<String?> _getCompressedImage(XFile originalFile, ChecklistItem item, String? instanceShipName, String? instancePort, DateTime? instanceDate) async {
    try {
      final Directory appDir = await getApplicationDocumentsDirectory();
      final String imageDir = '${appDir.path}/checklist_images';
      await Directory(imageDir).create(recursive: true);
      final String fileExtension = originalFile.path.split('.').last.toLowerCase();
      final String shipPart = _sanitizeForFilename(instanceShipName, defaultVal: 'ShipUnknown');
      final String portPart = _sanitizeForFilename(instancePort, defaultVal: 'PortUnknown');
      final String itemOrderPart = item.order.toString();
      final String datePart = instanceDate != null ? "${instanceDate.year}${instanceDate.month.toString().padLeft(2, '0')}${instanceDate.day.toString().padLeft(2, '0')}" : "DateUnknown";
      final String uniqueFileName = '${shipPart}_${portPart}_ITEM_${itemOrderPart}_${datePart}_${DateTime.now().millisecondsSinceEpoch}.$fileExtension';
  ...
  Future<void> _handleChecklistItemPhoto(ChecklistItem item, ChecklistItemResponse currentResponse) async {
    ...
    if (existingPhotoPath != null && existingPhotoPath.isNotEmpty) {
      action = await _showViewOrManageChecklistItemPhotoDialog(existingPhotoPath);
      ...
      if (action == 'delete') {
        try {
          ...
          if (!mounted) return;
          debugPrint(S.of(context).errorDeletingPhoto);
        }
    ...
    try {
      final String? newCompressedPath = await _getCompressedImage(pickedFile, item, _instance?.shipName, _instance?.port, _instance?.date);
      if (newCompressedPath != null && mounted) {
        ...
        if (oldPath != null && oldPath.isNotEmpty && oldPath != newCompressedPath) {
          try {
